...
public class AppleStoreService {
    ...
    // 限制实例
    init () {
        ICC_Logger.debug("AppleStoreService.init()")
        // 实例属性
        ...
        if let files = try? FileManager.default.subpathsOfDirectory(atPath: self.storageDirectory) {
            ICC_Logger.info("AppleStoreService.transactionFiles[%d]", files.count)
    ...
    public func request (productIdentifier:String, amount:Int, tradeNo:String, notifyUrl:String, observer:@escaping (_ code:Int, _ message:String,  _ transactionIdentifier:String?) -> Void) throws {
        ICC_Logger.info("AppleStoreService.request(productIdentifier:%@, amount:%d)", productIdentifier, amount)
        // 检查支持
    ...
    func startTransaction () {
        ICC_Logger.debug("AppleStoreService.startTransaction(productIdentifier:%@)", self.lastProductIdentifier!)
        objc_sync_enter(self)
        let txn = [
    ...
    func cancelTransaction (productIdentifier:String) {
        ICC_Logger.debug("AppleStoreService.startTransaction(productIdentifier:%@)", productIdentifier)
        objc_sync_enter(self)
    ...
    func finishTransaction (transactionIdentifier:String, productIdentifier:String) {
        ICC_Logger.debug("AppleStoreService.finishTransaction(productIdentifier:%@)", productIdentifier)
        objc_sync_enter(self)
        // 寻找相似文件
    ...
    func dispatchObserver(_ code:Int, _ message:String,  _ transactionIdentifier:String?) {
        objc_sync_enter(self)
        ICC_Logger.info("AppleStoreService.dispatchObserver(%d, %@)", code, message)
    ...
    private class SPRequestDelegate : NSObject, SKProductsRequestDelegate {
        ...
        func productsRequest(_ request: SKProductsRequest, didReceive response: SKProductsResponse) {
            ICC_Logger.info("AppleStoreService.SKProductsRequestDelegate.productsRequest(SKProductsRequest, SKProductsResponse)")
            // 检查物品标识
    ...
    private class SPTransactionObserver : NSObject, SKPaymentTransactionObserver {
        ...
        func paymentQueue(_ queue: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
            ICC_Logger.info("AppleStoreService.SKPaymentTransactionObserver.paymentQueue(SKPaymentQueue, [SKPaymentTransaction])")
            for txn in transactions {
                switch txn.transactionState {
                case .failed: // 支付失败
                    ICC_Logger.error("payment error(%d), %@", (txn.error! as NSError).code, txn.error!.localizedDescription)
                    // 取消订单
                ...
                case .purchased: // 支付成功
                    ICC_Logger.debug("payment succeed, %s", txn.transactionIdentifier!)
                    // 异步通知
    ...
    private class Notifier {
        ...
        class Item {
            ...
            init(filename:String) throws {
                ICC_Logger.debug("AppleStoreService.Notifier.Item(%@)", filename)
                // 内容调整
            ...
            func request() -> Bool {
                ...
                // 发起请求
                ICC_Logger.debug("AppleStoreService.Notifer.Task.requert(%@)", (req.url?.absoluteString)!)
                ...
                let task:URLSessionDataTask = URLSession.shared.dataTask(with: req, completionHandler: { data, res, err in
                    ...
                    ICC_Logger.warn("AppleStoreService.Notifer.Task.requert() error %@", err?.localizedDescription ?? "nil")
                    // 恢复阻塞
                    semaphore.signal()
                })
                ...
                ICC_Logger.warn("AppleStoreService.Notifer.Task.requert() throw %@", self.responseText ?? "nil")
                return false
            }
            ...
            /// - Throws:
            func remove () throws {
                ICC_Logger.debug("AppleStoreService.Notifier.remove() to %@", self.filename)
                if complete != true {
        ...
        func add (fileURLWithPath filename:String) throws {
            ICC_Logger.debug("AppleStoreService.Notifier.add(%@)", filename)
