...
public class Transaction {
    ...
    // 单件对象
    public static let `default` = Transaction()
    ...
    func doPostAppleStroe (withJson str:String) {
        Logger.debug( "Transaction.doPostAppleStroe(%@)", str)
        do {
            // 解析请求
            ...
            try AppleStoreService.default.request(productIdentifier: productIdentifier!, amount: amount!, tradeNo: tradeNo!, notifyUrl: notifyUrl!, observer: { (_ code:Int, _ message:String,  _ transactionIdentifier:String?) -> Void in
                SDK.getInstance().evalJavascript(ScriptBuilder.dispatchEvent(type: "apple_store_result", data: [
                    "code": code,
                    "message": message,
            ...
            Logger.error("Transaction.doPostAppleStroe error(%d), %@", err.code, err.description)
