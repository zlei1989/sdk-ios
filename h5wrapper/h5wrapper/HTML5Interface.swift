...
/// 定义标准
@objc protocol HTML5JsExportProtocol: JSExport {
...
/// 实现代码
public class HTML5Interface: NSObject, HTML5JsExportProtocol {
    
    /// 告知游戏
    /// 初始化完成后调用
    func ready() {
        Logger.debug("API call ready()")
        SDK.getInstance().setActive(state: true)
        // 初始支付自动补单
    ...
    /// 弹出窗口
    func tip(_ message:String) {
        Logger.debug("API call tip(%@)", message)
        Toast.makeText(text: message, duration: Toast.LENGTH_SHORT)
    }
    
    /// AES 加密
    func aesEncrypt(_ seed:String, _ cleartext:String) -> String {
        return AES.encrypt(seed:seed, cleartext:cleartext)
    }

    /// AES 加密
    func aesDecrypt(_ seed:String, _ ciphertext:String) -> String {
        return AES.decrypt(seed: seed,ciphertext: ciphertext)
    }

    /// 判断文件是否存在
    func fileExists(_ path:String) -> Bool {
        return IO.fileExists(path: path)
    }

    /// 写入文件
    func writeFile(_ filename:String, _ contents:String) -> Bool {
        return IO.writeFile(filename: filename, contents: contents)
    }

    /// 追加文件
    func appendFile(_ filename:String, _ contents:String) -> Bool {
        return IO.appendFile(filename: filename, contents: contents)
    }

    /// 读取文件
    func readFile(_ filename:String) -> String? {
        return IO.readFile(filename: filename)
    }
    
    /// 删除文件
    func deleteFile(_ path:String) -> Bool {
        return IO.deleteFile(path: path)
    }

    /// 获得文件列表
    func getFiles(_ path:String) -> [String] {
        return IO.getFiles(path: path)
    }
    
    /// 显示窗口
    func createActivity() {
        SDK.getInstance().createHtmlActivity()
    }
    
    /// 隐藏窗口
    func finishActivity() {
        SDK.getInstance().finishHtmlActivity()
    }
    
    /// 显示浮标按钮
    func enableAssistiveTouch(_ imageBase64PNG:String) {
        SDK.getInstance().setAssistiveTouch(
    ...
    /// 隐藏浮标按钮
    func disableAssistiveTouch() {
        SDK.getInstance().removeAssistiveTouch()
    }

    /// 唤起苹果支付
    func tranWithAppleStore(_ contextJSON:String) {
        Transaction.default.doPostAppleStroe(withJson: contextJSON)
    }
    ...
    func getMACAddress() -> String {
        return Device.current.macAddress
    }
    ...
    func getNetworkType() -> Int {
        return Device.current.networkType.rawValue
    }
    ...
    func getSimOperator() -> String? {
        return Device.current.simOperator
    }

    /// 获得Sim序列号
    func getSimSerialNumber() -> String? {
        return Device.current.simSerialNumber
    }
...
//        AudioServicesPlaySystemSound(.)
    ...
    func callback(_ key:String, _ resultJSON:String) -> Bool {
        ...
        if Thread.isMainThread {
            res = SDK.getInstance().executeCallback(key: key, resultJSON: resultJSON)
        } else {
...
class JsConsole: NSObject, ConsoleJsExportProtocol {
    ...
    /// 警告
    func warn(_ msg: String?) {
        Logger.warn("HTML5 %@", msg!)
    }

    /// 错误
    func error(_ msg: String?) {
        Logger.error("HTML5 %@", msg!)
    }

    /// log信息
    func log(_ msg: String?) {
        Logger.info("HTML5 %@", msg!)
    }
