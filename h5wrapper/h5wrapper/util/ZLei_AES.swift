...
class AES {
    ...
    static func encrypt (seed:String, cleartext:String) -> String {
        ...
        if UInt32(res) == UInt32(kCCSuccess) {
            ...
            Logger.error("ASE.encrypt(%@, %@) throw err(%d)", seed, cleartext, res)
            return ""
        }
    ...
    static func decrypt (seed:String, ciphertext:String) -> String {
        ...
        if strDat == nil {
            Logger.error("ASE.decrypt(%@, %@) throw base64 decode err", seed, ciphertext)
            return ""
        }
        ...
        if UInt32(res) == UInt32(kCCSuccess) {
            ...
            Logger.error("ASE.decrypt(%@, %@) throw err(%d)", seed, ciphertext, res)
            return ""
        }
