...
public class AssistiveTouch {
    ...
    /// 触摸事件监听
    private var _touchListener:AssistiveTouchListener?
...
//        self._window.isUserInteractionEnabled = true
    ...
    func setTouchListener (listener:AssistiveTouchListener) {
        self._touchListener = listener
    }
    
    /// 浮标淡出
    func fadeOut () {
        Logger.debug("AssistiveTouch.fadeOut()")
        // 贴边动画
        dragPropsStop()
        // 淡化效果
    ...
    /// 浮标淡入
    func fadeIn () {
        Logger.debug("AssistiveTouch.fadeIn()")
        // 淡化效果
        let anim = CABasicAnimation(keyPath: "opacity")
        anim.toValue = 1
        anim.duration = 0.2
    ...
    /// 浮标移动到屏幕边缘
    func dragPropsStop () {
        Logger.debug("AssistiveTouch.dragPropsStop()")
        // 计算最小距离
        let endOrigin = self.getOrigin()
        let origin = VelocityTracker.getShortestDistance(origin: endOrigin, bound: self.stopBound)
        // 计算动画时长
        ...
        // 打印日志
        Logger.debug("dragPropsStop - startX: %.2f, startY: %.2f, targetX: %.2f, targetX: %.2f, duration:%.2f", endOrigin.x, endOrigin.y, origin.x, origin.y, duration)
        // 通知监听
    ...
    func dragThrowProps (scroll:VelocityTracker.Scroll?) {
        Logger.debug("AssistiveTouch.dragThrowProps(VelocityTracker.Scroll?)")
        if scroll == nil {
            self.fadeOut()
            return
        }
        ...
        self.startAnimation(anim: anim)
        // 输出日志
        Logger.debug("dragThrowProps - radians: %.4f, distance: %.2f, duration: %.2f", scroll!.radians, scroll!.distance, scroll!.duration)
        Logger.debug("dragThrowProps - startX: %.2f, startY: %.2f, targetX: %.2f, targetY: %.2f", scroll!.x, scroll!.y, scroll!.targetX, scroll!.targetY)
    }
    ...
    func startAnimation (anim:CABasicAnimation) {
        ...
        Logger.debug("AssistiveTouch.startAnimation %d", anim.hash)
        self._window.layer.removeAllAnimations()
    ...
    class TouchMoveListener : UIViewController {
        ...
        /// 回弹惯性计算
        var velocityTracker:VelocityTracker? = VelocityTracker()
//        var velocityTracker:VelocityTracker? = nil
        ...
        override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
            ...
            //
            Logger.debug("AssistiveTouch drag start, x:%.2f, y:%.2f, offsetX:%.2f, offsetY:%.2f", self.startX, self.startY, self.offsetX, self.offsetY)
            // 停止动画
        ...
        override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
            let target = self.assistiveTouch._window
            Logger.debug("AssistiveTouch drag stop, x:%.2f, y:%.2f", target.layer.position.x, target.layer.position.y)
            // 打开浮标
            if self.isMoved != true {
                Logger.debug("AssistiveTouch touch(pin)")
