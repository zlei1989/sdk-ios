...
public class SDKView {
    ...
    func evalJavascript(_ script:String){
        Logger.debug("SDKView.evalJavascript(%@)", script);
        // 追加执行队列
    ...
    func evalJavascript () {
        if (self.isActive() != true){
            Logger.warn("SDKView.IsActive() equal to false")
            Toast.makeText(text: Constants.SDK_TEST_STARTING, duration: Toast.LENGTH_SHORT)
            return
        }
        Logger.debug("SDKView.evalJavascript(), Queus:%d", self._evalQueus.count)
        objc_sync_enter(self._evalQueus)
    ...
    func clearJavascript () {
        Logger.debug("SDKView.clearJavascript()")
        objc_sync_enter(self._evalQueus)
